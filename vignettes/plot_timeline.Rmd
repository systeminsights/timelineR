...
The `plot_timeline` function in R helps visualize multi-variate time-series having numeric and factor variables. The data used for demonstration is the pollution levels in Delhi, which is downloaded from http://www.cpcb.gov.in/ .

The structure of the input data frame should have one column of the type `POSIXct`. This column is used as the `timestamp` column.

```{r}
data_path <- system.file("extdata/delhi_air_pollution.csv", package = "timelineR")
air_pollution <- read.csv(data_path)
...
The default confifuration of the `plot_timeline` package plots univariate time series for all the variables in the order they appear in the data frame. It returns a grob object.
...
The columns for which the data should be plotted can be subsetted by passing the names of the column in the argument `data_cols`. By default, the data for all the variables is plotted.
...
color_mapping = list("odd_even" = c("NORMAL" = "blue", "ODD-EVEN" = "red"))
